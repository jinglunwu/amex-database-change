CREATE OR REPLACE FUNCTION staging.mi_event_log_transform_by_id(start_id bigint, end_id bigint)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
DECLARE r_row_count int8 default 0;
BEGIN	
	...
	FROM raw.movableink 
	WHERE id BETWEEN start_id AND end_id AND
	data->>'app_type'= 'live_pic' AND 
	data->>'event_type' IN ('open', 'open|visit', 'open|glance|visit', 'click', 'click|u_click')
	ON CONFLICT DO NOTHING;
	
	INSERT INTO backup.movableink_history3 SELECT * FROM raw.movableink WHERE id >= start_id AND id <= end_id;
	
	DELETE FROM raw.movableink WHERE id >= start_id AND id <= end_id;	
